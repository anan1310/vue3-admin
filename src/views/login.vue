...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid: boolean) => {
    if (valid) {
      if (param.password === "123123") {
        ElMessage.success('登录成功');
        localStorage.setItem('ms_username', param.username); //设置登录的用户名称
        const keys = permiss.defaultList[param.username == 'admin' ? 'admin' : 'user']; //通过登录的用户名确定当前的权限
